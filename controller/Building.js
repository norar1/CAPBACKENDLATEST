...
const getBuildingEmailTemplate = (status, permitData) => {
  ...
  switch (status) {
    case 'approved':
      return {
        subject: 'Building Permit Approved - Control No: ' + control_no,
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <h2 style="color: #28a745;">🎉 Building Permit Approved!</h2>
            <p>Dear ${owner_establishment},</p>
            <p>We are pleased to inform you that your building permit application has been <strong style="color: #28a745;">APPROVED</strong>.</p>
            <div style="background-color: #f8f9fa; padding: 20px; border-left: 4px solid #28a745; margin: 20px 0;">
              <h3>Permit Details:</h3>
              <ul style="list-style: none; padding: 0;">
                <li><strong>Control Number:</strong> ${control_no}</li>
                <li><strong>Building Owner:</strong> ${owner_establishment}</li>
                <li><strong>Location:</strong> ${location}</li>
                <li><strong>Status:</strong> <span style="color: #28a745;">Approved</span></li>
              </ul>
            </div>
            <p>You can now proceed with your building construction/renovation. Please keep this permit information for your records.</p>
            <p>Thank you for your compliance with local building regulations.</p>
    ...
    case 'rejected':
      return {
        subject: 'Building Permit Application Update - Control No: ' + control_no,
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <h2 style="color: #dc3545;">❌ Building Permit Application Update</h2>
            <p>Dear ${owner_establishment},</p>
            <p>We regret to inform you that your building permit application has been <strong style="color: #dc3545;">REJECTED</strong>.</p>
    ...
    default:
      return {
        subject: 'Building Permit Status Update - Control No: ' + control_no,
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <h2 style="color: #ffc107;">📋 Building Permit Status Update</h2>
            <p>Dear ${owner_establishment},</p>
            <p>Your building permit application status has been updated to: <strong style="color: #ffc107;">${status.toUpperCase()}</strong></p>
...
export const CreatePermit = async (req, res) => {
  try {
    ...
    const existingPermit = await Building.findOne({
      owner_establishment,
      location,
      control_no
    });
    ...
    if (existingPermit) {
      return res.status(409).json({ message: "Building permit already exists", success: false });
    }

    const newPermit = new Building({
      user: req.user.id,
      date_received,
      owner_establishment,
      location,
    ...
    res.status(201).json({ message: "Building permit created successfully!", success: true });
  } catch (error) {
...
export const UpdatePermit = async (req, res) => {
  try {
    ...
    const permit = await Building.findById(id);
...
export const UpdateStatus = async (req, res) => {
  try {
    ...
    const permit = await Building.findById(id).populate('user', 'email');
...
export const UpdatePaymentStatus = async (req, res) => {
  try {
    ...
    res.status(200).json({ 
      message: `Building payment status updated to ${payment_status}`, 
      success: true 
    });
  } catch (error) {
...
export const GetPermit = async (req, res) => {
  try {
    const permits = await Building.find().populate("user");
...
export const GetPermitsByStatus = async (req, res) => {
  try {
    ...
    const permits = await Building.find({ status }).populate("user");
...
export const SearchPermits = async (req, res) => {
  try {
    ...
    const permits = await Building.find({
      $or: [
